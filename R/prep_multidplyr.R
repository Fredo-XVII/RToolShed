...
#   var_grps <- .data %>%
#     dplyr::select(...) # .data$location,.data$vd,.data$node_td
#
#   .data$n_grps <- rep(1:n_cores, length.out = nrow(var_grps))
#
#   .data$core_grps <- bind_cols(tibble(.data$n_grps), var_grps)
#
...
#     multidplyr::partition(.data$n_grps, cluster = cluster) # NOTE:.data$ is not used because it bombs.
#   return(id_cluster)
# }
